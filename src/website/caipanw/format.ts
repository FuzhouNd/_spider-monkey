...
(async () => {
  const sourceDataList = (await readCsv('./data/test.csv')) as { title: string; info: string; content: string }[];
  ...
  for (const data of sourceDataList) {
    ...
    await writeCsv('./data/tFor_2022_09_28.csv', [{ no, year, province, city, county, court, cause, causeNo, content:data.content.replace(/[\r\n]+/g, ' ') }])
  }
})();
